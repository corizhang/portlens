...
### 阶段 P2-3：复用 `ApplyEntries` 中间集合
- **状态：** complete
- **开始时间：** 2026-07-07
- **完成时间：** 2026-07-07
- 执行的操作：
  - 在 `MainWindowViewModel` 中添加 `_liveKeysBuffer`（`HashSet<PortEntryKey>`）和 `_orderedBuffer`（`List<PortEntryViewModel>`）
  - 在 `ApplyEntries` 中先 `Clear()` 再复用，避免每轮扫描新建集合
  - 保持 VM 实例复用和 `ResetTo` 批量更新逻辑不变
...
- 测试结果：
  ...
  | 发布 exe 启动 | `scripts/smoke-test.ps1` | 窗口正常显示 | PID=31636，Children=0，Smoke test passed | 通过 |
- 下一步：
  - 实施 P3-1：托盘菜单缓存与 About 徽章缓存

### 阶段 P2-2：`ProjectRootResolver` 目录 marker 缓存
- **状态：** complete
- **开始时间：** 2026-07-07
- **完成时间：** 2026-07-07
- 执行的操作：
  - 在 `ProjectRootResolver` 中添加 `ConcurrentDictionary<string, CacheEntry>` 缓存，按输入目录缓存解析后的项目根，TTL 5 秒
  - 调整 `HasRootMarker` 检查顺序：`.git` → `package.json` workspaces → `.vscode` → `.idea` → `*.sln` → 其他 workspace/language marker
  - 保持聚合分组与 workspace 语义不变
  - 运行 `dotnet build PortLens.sln` 验证（0 警告，0 错误）
  - 运行 `dotnet test PortLens.sln` 验证（61 个测试全部通过）
  - 运行 `scripts/publish.ps1` 发布
  - 运行 `scripts/smoke-test.ps1` 验证发布后的 `PortLens.exe` 窗口正常显示
...
- 测试结果：
  ...
  | 构建 | `dotnet build PortLens.sln` | 成功 | 0 警告 0 错误 | 通过 |
  | 单元测试 | `dotnet test PortLens.sln` | 全部通过 | 61 个测试通过，0 失败 | 通过 |
  | 发布 exe 启动 | `scripts/smoke-test.ps1` | 窗口正常显示 | PID=17992，Children=0，Smoke test passed | 通过 |
- 下一步：
  - 实施 P2-3：复用 `ApplyEntries` 中间集合
...
### 阶段 P3-2：命令行空白归一化去 Regex
- **状态：** complete
- **开始时间：** 2026-07-07
- **完成时间：** 2026-07-07
- 执行的操作：
  - 新增 `CommandLineNormalizer` 静态类，使用 `StringBuilder` 实现空白折叠归一化
  - 在 `ProcessCommandLineReader` 中移除 `Regex.Replace` 与 `CommandLineRegex` 字段，改为调用 `CommandLineNormalizer.Normalize`
  - 处理 null/空/仅空白输入时返回 null，保留原有语义
  - 新增 `CommandLineNormalizerTests`，覆盖首尾空白、Tab、换行、多空格、单空格保留等场景
  - 运行 `dotnet build PortLens.sln` 验证（0 警告，0 错误）
  - 运行 `dotnet test PortLens.sln` 验证（74 个测试全部通过）
  - 运行 `scripts/publish.ps1` 发布
  - 运行 `scripts/smoke-test.ps1` 验证发布后的 `PortLens.exe` 窗口正常显示
- 创建/修改的文件：
  - `work/PortLens.Core/Services/CommandLineNormalizer.cs`（新增）
  - `work/PortLens.Core/Services/ProcessCommandLineReader.cs`
  - `work/PortLens.Core.Tests/CommandLineNormalizerTests.cs`（新增）
  - `task_plan.md`
  - `progress.md`
- 测试结果：
  | 测试 | 输入 | 预期结果 | 实际结果 | 状态 |
  ...
  | 发布 exe 启动 | `scripts/smoke-test.ps1` | 窗口正常显示 | PID=35916，Children=0，Smoke test passed | 通过 |
- 下一步：
  - 实施 P3-3：`AppMetricsTimer` 后台暂停

### 阶段 P3-3：`AppMetricsTimer` 后台暂停
- **状态：** complete
- **开始时间：** 2026-07-07
- **完成时间：** 2026-07-07
- 执行的操作：
  - 在 `MainWindow.xaml.cs` 中新增 `UpdateAppMetricsTimer`，当窗口隐藏或最小化时停止 `_appMetricsTimer`，恢复时重新启动并刷新指标
  - 恢复时重新采样当前进程的 `TotalProcessorTime` 作为基准，避免将隐藏期间的 CPU 时间计入首次读数
  - 在 `StateChanged` 与 `IsVisibleChanged` 事件中同步调用 `UpdateAppMetricsTimer`
  - 运行 `dotnet build PortLens.sln` 验证（0 警告，0 错误）
  - 运行 `dotnet test PortLens.sln` 验证（74 个测试全部通过）
  - 运行 `scripts/publish.ps1` 发布
  - 运行 `scripts/smoke-test.ps1` 验证发布后的 `PortLens.exe` 窗口正常显示
